...
//Open and closed values for actuators. Changing these values here will change them in the rest of the code where they are called
int open = 1150;
int close = 1690;

int dT = 5; // time in seconds for dPdT calculations. Change it here to change it everywhere in the code where dT is used.


//These variables are used to calculate elapsed time, pressure, and dPdT for the opening of the valve when switch is in the armed position
long startTime;
int elapsedTime;
int32_t pressure;
int32_t pressure1;
...
void loop() { 
  ...
  if(armOn == LOW){  
    ...
    while(cycle_complete == false){
      ...
      if((double)(millis() - startTime) >= 360 || (int32_t)pressure <= 13000){ //3600000
